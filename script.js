...
// move logo into header when scrolling past hero
; (function () {
  const photo = document.querySelector('.photo');
  const originalWrap = photo.parentElement;
  const header = document.getElementById('header');
  // when page1 bottom hits the top of the header:
  const threshold = document.getElementById('page1')
    .getBoundingClientRect().bottom
    - header.offsetHeight;

  window.addEventListener('scroll', () => {
    if (window.scrollY > threshold) {
      if (!header.contains(photo)) {
        header.appendChild(photo);
        photo.classList.add('in-header');
      }
    } else {
      if (!originalWrap.contains(photo)) {
        originalWrap.appendChild(photo);
        photo.classList.remove('in-header');
      }
    }
  });
})();
